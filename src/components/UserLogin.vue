    ...
    <el-dialog title="账号登录" :visible.sync="logshow" width="500px" @closed="closeDialog">
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" id="loginForm">
        ...
        <el-form-item prop="inputVerificationCode" v-if="errorCount>2">
          <el-input
            placeholder="请输入验证码"
            v-model="ruleForm.inputVerificationCode"
            class="identifying"
          ></el-input>
          <div @click="createCode" class="verificationCode">
            <p>
              <span :style="{'color': color1 }">{{code1}}</span>
              <span :style="{'color': color2 }">{{code2}}</span>
              <span :style="{'color': color3 }">{{code3}}</span>
              <span :style="{'color': color4 }">{{code4}}</span>
            </p>
          </div>
        </el-form-item>
